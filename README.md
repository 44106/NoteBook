...
目录的前两级会显示为“分类 / 课程”。专业课按学期归档，非专业课统一放在 `非专业课/` 下。建议按下面的形式新增资料：
...
根目录的 `index.html`、`app.js`、`styles.css`、`site-data.js`、本 README 和 `scripts/` 是网站或维护文件，不会出现在网页资料列表中。不要把网页构建文件、临时文件或个人配置放进上述资料根目录。

## 上传资料

1. 在对应学期或 `非专业课` 根目录下找到或新建课程目录，专业课课程名建议包含学分，便于识别。
...
6. 提交并推送资料和更新后的 `site-data.js`：
   ...
   ```powershell
   git add "大二春夏" "大二秋冬" "非专业课" site-data.js
   git commit -m "补充课程资料"
   git push origin main
   ```

   资料属于非专业课时，将文件放入 `非专业课`；也可以在 GitHub 网页上传文件，但仍需在本地运行索引脚本并提交新的 `site-data.js`，以保证离线或 API 不可用时网站资料完整。
...
- `site-data.js` 是静态索引，直接由当前工作区的三个资料根目录生成。
- 网页加载后会尝试读取 GitHub 仓库树作为实时索引；实时索引也只接受 `大二春夏/`、`大二秋冬/` 和 `非专业课/` 下的文件。
