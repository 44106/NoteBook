...
1. 先看“本章解决什么问题”，知道这章在整门课中的位置；
2. 再看定义和核心定理，理解每个符号是什么意思；
3. 跟着算法步骤做一遍小矩阵手算；
4. 最后背“考场模板”和“易错点”。

如果时间很紧，优先顺序是：

1. LU/PLU、Cholesky/LDL^T、最小二乘、Householder/Givens；
2. 范数、条件数、投影矩阵、证明题模板；
3. Jacobi/G-S/SOR、CG；
4. Hessenberg、QR、Schur、SVD、幂法、二分法。

整门课可以看成四句话：

- 解线性方程组：把困难矩阵变成三角矩阵；
- 解最小二乘：把残差问题变成正交投影或 QR；
- 解迭代问题：把误差写成 `e^{(k+1)}=Be^{(k)}`，然后看 `rho(B)<1`；
- 解特征值问题：用正交相似变换保持特征值，同时逐步逼近三角形结构。
...
当 `m=n` 且 `A` 非奇异时，方程组有唯一解：

\[
x=A^{-1}b.
\]

考试中很少真的让你求 `A^{-1}` 再乘 `b`，因为这样数值上不经济。真正常用的是：
...
- `R(A)`：矩阵 `A` 的列空间，也就是所有 `Ax` 组成的空间；
- `N(A)`：矩阵 `A` 的零空间，也就是所有满足 `Ax=0` 的向量；
- `rank(A)`：列空间维数，也等于行空间维数；
- `A` 满列秩：`rank(A)=n`，列向量线性无关；
- `A` 满秩方阵：`rank(A)=n`，等价于 `A` 可逆。
...
因此 `Ax=0`。这个结论的用处是：如果 `A` 满列秩，那么 `A^TA` 非奇异，最小二乘正则方程有唯一解。
...
1. 所有特征值都大于 0；
2. 所有顺序主子式都大于 0；
3. 能做 Cholesky 分解且所有对角元为正；
4. 二次型 `x^TAx` 可以配方成正平方和。

顺序主子式判别的意思是：对称矩阵 `A` 正定当且仅当

\[
\det A_1>0,\quad \det A_2>0,\quad \cdots,\quad \det A_n>0,
\]

其中 `A_k` 是 `A` 左上角 `k` 阶主子矩阵。

SPD 的三个重要后果：

- 可以做 Cholesky；
- 共轭梯度法适用；
- 对任意非零 `x`，`x^TAx` 可以作为一种“能量”或“A-范数”的平方。
...
如果 `L` 是下三角，解 `Ly=b` 可以前代；如果 `U` 是上三角，解 `Ux=y` 可以回代。
...
基本思路是：用消元把 `A` 变成上三角 `U`，同时把消元过程记录成下三角 `L`。最终得到：
...
以第 `k` 列为例，消去主元下面的元素。设当前主元是 `a_{kk}^{(k)}`，要消去第 `i` 行第 `k` 列元素，乘子为：
...
这些乘子 `l_{ik}` 放进 `L` 的下三角部分，上三角结果就是 `U`。
...
所以必须先把右端项也换行。常见失分点就是忘记 `Pb`。

### 5. 列主元 PLU

列主元 Gauss 消元每一步在当前列中，从第 `k` 行到第 `n` 行选绝对值最大的元素作为主元。
...
这类题的关键不是死背矩阵，而是知道每个 `L` 中的数来自哪里。

### 7. 全主元 Gauss 消元

全主元每一步不仅换行，也换列。第 `k` 步在剩余子矩阵中选绝对值最大的元素作为主元。
...
全主元比列主元更稳定，但实现更麻烦。纸面考试如果考全主元，多半是让你理解 `Q` 代表列交换，也就是未知量顺序交换。

### 8. Gauss-Jordan 求逆

Gauss-Jordan 的目标是把 `A` 化成单位阵：
...
设 `A` 是 SPD，则存在唯一的下三角矩阵 `L`，且对角元为正，使得：
...
它的优点是避免开平方。对精确手算题也常更干净，因为很多数可以留在 `D` 中。
...
题目让“证明 `A` 正定”，不要上来就分解。先证明正定，再用分解。
...
几何理解：`Ax` 永远在列空间 `R(A)` 里。我们要找列空间里离 `b` 最近的向量。
...
如果 `P` 是非零正交投影，则特征值只可能是 0 或 1，因此：
...
如果题目说“存在矩阵 `X`，使得对任意 `b`，`x=Xb` 都是最小二乘解”，那么 `AX` 是投影矩阵。常见结论：

\[
AXA=A,\quad (AX)^T=AX.
\]

理解：`AXb` 是 `b` 在 `R(A)` 上的正交投影，所以 `AX` 对称且幂等，并且在 `R(A)` 上保持不变。
...
对矩阵 `A`，第 1 步取第一列下方向量，构造 Householder 把第一列第 2 到第 `m` 个元素消掉。
...
### 8. Householder 与 Givens 对比

Householder：

- 一次消掉一串；
- 适合稠密矩阵；
- 常用于 QR 和 Hessenberg 化。
...
## 七、定常迭代法：Jacobi、Gauss-Seidel、SOR
...
也就是说，如果原矩阵非对角元素是 `a_{ij}`，那么 `L` 和 `U` 中往往带负号。不同教材可能写 `A=D+L+U`，所以一定要跟题目或讲义约定一致。
...
逐行理解：第 `i` 个分量更新时，前面已经更新过的分量用新值，后面还没更新的分量用旧值。
...
若 `A` 严格对角占优，则 Jacobi 和 Gauss-Seidel 通常收敛。

若 `A` SPD，则 Gauss-Seidel 收敛。
...
CG 的关键是选择一组 A-共轭方向。沿这些方向逐步最小化，理论上 `n` 步内得到精确解。
...
## 九、特征值问题：幂法、Hessenberg、QR、Schur
...
- 矩阵不可对角化且有 Jordan 块；
- 初始向量恰好没有主特征向量分量；
- 最大特征值有相同模但符号相反，例如 `lambda` 和 `-lambda`。
...
因为若 `A` 的特征值是 `lambda_i`，则 `(A-\mu I)^{-1}` 的特征值是：
...
1. 取第 `k` 列从第 `k+1` 行到第 `n` 行的向量；
2. 构造 Householder，把该向量除第一个分量外消为 0；
3. 嵌入成：
...
- QR 分解可用局部 Givens 或 Householder 完成；
- 每步只消次对角以下元素；
- 乘回后不会产生更低位置的非零元；
- 所以结构保持。
...
其中 `Q` 酉，`T` 上三角。`T` 的对角线元素就是 `A` 的特征值。
...
其中 `Q` 正交，`T` 是拟上三角矩阵。拟上三角允许 `1` 阶和 `2` 阶对角块，`2` 阶块对应一对共轭复特征值。

Schur 分解与 QR 迭代的关系：

- QR 迭代通过正交相似变换逐步逼近 Schur 形；
- 对称矩阵的 Schur 形可以进一步成为对角阵；
- 因此对称 QR 最终逼近特征值对角阵。
...
## 十、SVD、Moore-Penrose 逆、Jacobi 与二分法
...
其中 `U,V` 正交，`\Sigma` 是对角型矩阵，非负对角元为奇异值：
...
选择一个非对角元素 `a_{pq}`，构造 Givens 型旋转 `J`，使：
...
- 写 `Q^TAQ=H`；
- 每步用 Householder 只处理活动子块；
- 这是正交相似变换，所以特征值不变。

若题目问 QR：

- 写 `A_k=Q_kR_k`；
- 写 `A_{k+1}=R_kQ_k=Q_k^TA_kQ_k`；
- 说明相似变换保持特征值。

若题目问 Schur：

- 复 Schur：`A=QTQ^*`，`T` 上三角；
- 实 Schur：`A=QTQ^T`，`T` 准上三角；
- QR 迭代逼近 Schur 形。
...
1. 证明相似诱导范数是矩阵范数；
2. 给 3 阶矩阵做 LU/PLU 并解方程；
3. 最小二乘正则方程加 Cholesky；
4. Householder 与 Givens 手算；
5. 参数矩阵正定性与 Jacobi/G-S 收敛；
6. Jacobi 迭代矩阵和谱半径；
7. A-共轭向量线性无关与 `A^{-1}` 展开；
8. Hessenberg、QR、Schur、SVD 理论综合。

中等优先级：

1. Frobenius 范数和正交不变性；
2. 投影矩阵 `P=A(A^TA)^{-1}A^T`；
3. 追赶法；
4. 条件数与残量误差；
5. 幂法、逆迭代、位移 QR；
6. Jacobi 特征值方法与二分法。
...
1. 手算一遍 LU/PLU；
2. 手算一遍 Cholesky 或 LDLT；
3. 手算一遍最小二乘正则方程；
4. 手算一遍 Householder/Givens；
5. 写出 Jacobi/G-S/SOR 三个迭代矩阵；
6. 默写 CG 递推；
7. 默写 QR 迭代与 Schur 分解；
8. 背 5 个证明模板：范数、正定、收敛、投影、A-共轭。
