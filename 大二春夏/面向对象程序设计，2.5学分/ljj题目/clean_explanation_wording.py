...
REPLACEMENTS = {
    ...
    "题库化表述，注意 abstraction 不是单纯 data hiding。": "这是常见教材表述；注意 abstraction 不只是 data hiding。",
    "临时对象和异常会让计数题更复杂，但题库按常规情况处理。": "这里按构造成功、正常生命周期结束的常规对象计数理解。",
    "题库原句有语法问题，但意思如此。": "原英文表述不自然，核心是引用返回已有对象。",
    "题库把零参数构造函数称为默认构造函数。": "零参数构造函数是默认构造函数的一种典型形式。",
    "现代编译器可优化，但题库按传统语义考。": "现代编译器可做返回值优化，但概念上可理解为按值返回产生结果对象。",
    "题库把 `ClassName functionName(){}` 视为对象返回函数的正确写法。": "`ClassName functionName(){}` 表示函数返回一个类类型对象。",
